...
def get_colors(colorscheme):
    colors = {"primary": hexFromArgb(colorscheme.get_primary()),
                    ...
                    "surfaceVariant": hexFromArgb(colorscheme.get_background()),
                    "onSurfaceVariant": hexFromArgb(colorscheme.get_onSurfaceVariant()),
                    "outline": hexFromArgb(colorscheme.get_outline()),
                    "shadow": hexFromArgb(colorscheme.get_primaryContainer()),
...
def setup(img):
    ...
    os.system(f"swww img {img} --transition-fps 165 --transition-type wipe --transition-duration 2")
...
def emoji_wallpaper(colors_list):
    ...
    emoji_color = colors_list['primary']

    # Define a font and font size for the emojis (larger size)
    font = ImageFont.truetype("/usr/share/fonts/TTF/SymbolsNerdFont-Regular.ttf", 128)
